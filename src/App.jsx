  ...
  return (<div style={{ overflowX: "auto", borderRadius: 9, border: "1px solid #E0D5C1" }}><table style={{ borderCollapse: "collapse", width: "100%", minWidth: 520, fontSize: 15, fontFamily: "'Noto Sans TC', sans-serif" }}>
    ...
    <tbody>{SLOTS.map((time) => { const isH = time.endsWith(":00"); const isBr = time === "14:00"; return (<tr key={time} style={{ ...(isBr ? { borderTop: "3px solid #C2563A" } : {}) }}><td style={{ padding: "1px 3px", textAlign: "center", background: isH ? "#F0E8D8" : "#F8F2E6", position: "sticky", left: 0, zIndex: 5, borderRight: "2px solid #D4C5A9", borderTop: isH ? "1px solid #D4C5A9" : "1px solid #EDE5D5", fontWeight: isH ? 700 : 400, color: isH ? "#3D2B1F" : "#8B7355", fontSize: isH ? 12 : 11, height: 26 }}>{time}</td>
      {wd.map((date, di) => { const ds = dsArr[di]; const as = getStart(ds, time); const aa = getAt(ds, time); const occ = !!aa; const th = aa ? (TH_MAP[aa.therapist] || TH_MAP["X"]) : null;
        ...
        const notMyAppt = filterTh && occ && aa?.therapist !== filterTh;
        const showBlack = isBlocked || dim || notMyAppt;
        return (<td key={di} onClick={() => occ ? onApptClick(aa) : !dim && !isBlocked && onCellClick(date, time)} style={{ borderLeft: "1px solid #EDE5D5", borderTop: isH ? "1px solid #D4C5A9" : "1px solid #EDE5D5", padding: 0, height: 26, cursor: (dim || isBlocked) ? "default" : "pointer", position: "relative", background: showBlack ? "#2A2A2A" : occ ? `${th.color}18` : "#FFFDF5", opacity: 1 }} onMouseEnter={e => { if (!occ && !dim && !isBlocked) e.currentTarget.style.background = "#F0E0C8"; }} onMouseLeave={e => { if (!occ && !dim && !isBlocked) e.currentTarget.style.background = "#FFFDF5"; }}>{as && !notMyAppt && (()=>{ const asth = TH_MAP[as.therapist] || TH_MAP["X"]; return (<div style={{ position: "absolute", top: 1, left: 1, right: 1, height: (as.duration / 15) * 26 - 2, background: `linear-gradient(135deg, ${asth.color}EE, ${asth.color}AA)`, borderRadius: 3, zIndex: 2, padding: "2px 4px", color: "white", fontSize: 10, fontWeight: 600, overflow: "hidden", display: "flex", flexDirection: "column", border: as.checkedIn ? "2px solid #FFD700" : as.onDuty ? "none" : "1.5px dashed rgba(255,255,255,0.6)" }}><div style={{ display: "flex", justifyContent: "space-between" }}><span>{as.patient.slice(0, 3)}</span><span style={{ opacity: 0.8, fontSize: 9 }}>{thLabel(asth)}</span></div>{as.duration >= 30 && <span style={{ fontSize: 9, opacity: 0.75 }}>{as.duration}m·{as.onDuty ? "內" : "外"}</span>}</div>);})()}</td>); })}</tr>); })}</tbody>
  </table></div>);
}
