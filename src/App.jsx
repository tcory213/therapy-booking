...
import { collection, doc, onSnapshot, addDoc, deleteDoc, updateDoc, setDoc, writeBatch, query, where, getDocs, deleteField } from "firebase/firestore";
...
/* ═══════════════════════════════════════════ Admin Detail (main) ═══════════════════════════════════════════ */
function AdminDetail({ appt, appts, onClose, onDelete, onUpdate, onAlert, onCopyDates, onStartCopy, onAddExtra }) {
  ...
  return (<div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
    ...
    {showCopy && <div style={{ padding: "10px 12px", background: "#EEEEFF", borderRadius: 8, fontSize: 13, color: "#5B6ABF", fontWeight: 600 }}>請關閉此視窗後點選目標時段</div>}
    {!editing && !showCopy && <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
      ...
      <button onClick={() => { onStartCopy(appt); onClose(); }} style={{ flex: 1, padding: 9, borderRadius: 7, border: "1.5px solid #5B6ABF", background: "#EEEEFF", color: "#5B6ABF", cursor: "pointer", fontWeight: 600, fontSize: 14, fontFamily: "'Noto Sans TC', sans-serif" }}>📋 複製</button>
...
function LuAdminDayView({ appts, selDate, onCellClick, onApptClick, luSlotCfg, setLuSlotCfg }) {
  ...
  return (<div>
    <div style={{ display: "flex", gap: 12, marginBottom: 10, padding: "8px 14px", background: "#F8FDFB", borderRadius: 7, border: `1px solid ${LU_COLOR}30`, fontSize: 17, color: "#5A4A3A", flexWrap: "wrap", alignItems: "center" }}>
      ...
      <span style={{ marginLeft: "auto", fontSize: 13, color: "#8B7355" }}>點「開/關」切換時段開放</span>
    </div>
      ...
      {LU_SLOTS.map(time => { const isH = time.endsWith(":00"); const as = getStart(time); const aa = getAt(time); const occ = !!aa; const closed = isLuSlotClosed(ds, time, luSlotCfg);
        return (<tr key={time} style={{ cursor: "pointer" }}><td style={{ padding: "3px 8px", textAlign: "center", background: isH ? "#E8F5F0" : "#F2FAF7", borderRight: `2px solid ${LU_COLOR}30`, borderTop: isH ? `1px solid ${LU_COLOR}30` : "1px solid #E0EDE8", fontWeight: isH ? 700 : 400, color: isH ? LU_COLOR : "#6BA898", height: 38 }}>{time}</td>
          ...
          <td style={{ textAlign: "center", borderTop: isH ? `1px solid ${LU_COLOR}30` : "1px solid #E0EDE8" }}>
            <button onClick={() => toggleSlot(time)} style={{ padding: "2px 8px", borderRadius: 4, border: "none", cursor: "pointer", background: closed ? "#EDE5D5" : `${LU_COLOR}DD`, color: closed ? "#B5A898" : "white", fontSize: 12, fontWeight: 600, fontFamily: "'Noto Sans TC', sans-serif" }}>{closed ? "關" : "開"}</button>
...
function LuAdminWeekGrid({ appts, selDate, onCellClick, onApptClick, luSlotCfg }) {
  ...
  return (<div style={{ overflowX: "auto", borderRadius: 9, border: `1px solid ${LU_COLOR}40` }}><table style={{ borderCollapse: "collapse", width: "100%", minWidth: 320, fontSize: 13, fontFamily: "'Noto Sans TC', sans-serif" }}>
    <thead><tr><th style={{ padding: "7px 3px", background: LU_COLOR, color: "white", position: "sticky", left: 0, zIndex: 10, minWidth: 42, width: 42, borderRight: `2px solid ${LU_COLOR}`, fontSize: 13 }}>時間</th>
      {wd.map((date, di) => { const isT = dsArr[di] === todayStr; return (<th key={di} style={{ padding: "5px 2px", background: isT ? "#1D8068" : LU_COLOR, color: "white", borderLeft: `1px solid ${LU_COLOR}88`, fontSize: 12, minWidth: 80, width: "33%" }}><div>{`${date.getMonth() + 1}/${date.getDate()}`}</div><div style={{ fontSize: 12, color: isT ? "#A0E8D0" : "#88CCB8", marginTop: 1 }}>{LU_WDAY[di]}{isT && " 今"}</div></th>); })}</tr></thead>
    <tbody>{LU_SLOTS.map((time) => { const isH = time.endsWith(":00"); return (<tr key={time}><td style={{ padding: "1px 2px", textAlign: "center", background: isH ? "#E8F5F0" : "#F2FAF7", position: "sticky", left: 0, zIndex: 5, borderRight: `2px solid ${LU_COLOR}40`, borderTop: isH ? `1px solid ${LU_COLOR}30` : "1px solid #E0EDE8", fontWeight: isH ? 700 : 400, color: isH ? LU_COLOR : "#6BA898", fontSize: isH ? 13 : 12, height: 28 }}>{time}</td>
      {wd.map((date, di) => {
        ...
        return (<td key={di} onClick={() => occ ? onApptClick(aa) : !closed && onCellClick(date, time)}
          style={{ borderLeft: "1px solid #E0EDE8", borderTop: isH ? `1px solid ${LU_COLOR}30` : "1px solid #E0EDE8", padding: 0, height: 28, cursor: occ || !closed ? "pointer" : "default", position: "relative", background: closed ? "#F0EBE0" : occ ? `${LU_COLOR}15` : "#F8FDFB" }}
          onMouseEnter={e => { if (!occ && !closed) e.currentTarget.style.background = "#DDF0E8"; }} onMouseLeave={e => { if (!occ && !closed) e.currentTarget.style.background = "#F8FDFB"; }}>
          {as && (<div style={{ position: "absolute", top: 1, left: 1, right: 1, height: (as.duration / 15) * 28 - 2, background: `linear-gradient(135deg, ${LU_COLOR}EE, ${LU_COLOR}AA)`, borderRadius: 3, zIndex: 2, padding: "1px 3px", color: "white", fontSize: 14, fontWeight: 600, overflow: "hidden", display: "flex", flexDirection: "column", border: as.checkedIn ? "2px solid #FFD700" : "none" }}>
            <span>{as.patient.slice(0, 3)}</span>
            {as.duration >= 30 && <span style={{ fontSize: 12, opacity: 0.75 }}>{as.duration}m</span>}
          </div>)}
          {closed && !occ && <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, color: "#B5A898" }}>關</div>}
        </td>);
      })}</tr>); })}</tbody>
  </table></div>);
}
    ...
    <div style={{ background: "#FFFDF5", borderRadius: 10, border: "1px solid #E0D5C1", overflow: "hidden", marginBottom: selTh ? 16 : 0 }}>
      ...
      <div style={{ padding: 12, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 10 }}>
        {summaries.filter(s2 => !salaryViewer || salaryViewer === "all" || s2.id === salaryViewer).map(s2 => (
          <div key={s2.id} onClick={() => { setSelTh(selTh === s2.id ? null : s2.id); setShowCoDutyDetail(false); setShowTeamDetail(false); }}
      ...
      <div style={{ background: "#FFFDF5", borderRadius: 10, border: `1.5px solid ${selColor}40`, overflow: "hidden" }}>
        ...
        <div style={{ padding: "14px 16px", borderBottom: "1px solid #E0D5C1", fontSize: 12 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr auto auto", gap: "6px 14px", alignItems: "center" }}>
            ...
            {selTh !== "LU" && teamBonus[selTh] > 0 && (() => {
              const teamDetail = monthAppts.filter(a =>
                a.checkedIn && a.onDuty && getApptTreats(a).includes("manual") && a.therapist !== "X" && a.therapist !== selTh
              ).sort(sortByDateTime);
              return (<>
                <div style={{ color: "#2E7D6F", display: "flex", alignItems: "center", gap: 6 }}>
                  全團分潤（徒手5%）
                  <button onClick={e => { e.stopPropagation(); setShowTeamDetail(v => !v); }} style={{ fontSize: 10, padding: "1px 7px", borderRadius: 4, border: "1px solid #2E7D6F", background: showTeamDetail ? "#2E7D6F" : "#E8F5F0", color: showTeamDetail ? "white" : "#2E7D6F", cursor: "pointer", fontFamily: "'Noto Sans TC', sans-serif", fontWeight: 600 }}>{showTeamDetail ? "收起" : "明細"}</button>
                </div>
                <div />
                <div style={{ textAlign: "right", fontWeight: 700, color: "#2E7D6F" }}>+NT$ {salaryUnlocked ? teamBonus[selTh].toLocaleString() : "🔒"}</div>
                {showTeamDetail && <div style={{ gridColumn: "1/-1", background: "#E8F5F0", border: "1px solid #B8DDD4", borderRadius: 7, padding: "10px 12px", marginTop: 4 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: "#2E7D6F", marginBottom: 6 }}>全團分潤來源（{teamDetail.length} 筆）</div>
                  <table style={{ borderCollapse: "collapse", width: "100%", fontSize: 11, fontFamily: "'Noto Sans TC', sans-serif" }}>
                    <thead><tr style={{ background: "#D0EDE6" }}>{["日期","時間","患者","治療師","時長","收費","分潤(5%)"].map(h => <th key={h} style={{ padding: "3px 6px", textAlign: "left", fontWeight: 600, color: "#1A5A4A", borderBottom: "1px solid #B8DDD4" }}>{h}</th>)}</tr></thead>
                    <tbody>{teamDetail.map((a, i) => {
                      const rev = calcRevenue(getApptManualDur(a), "manual");
                      const th2 = TH_MAP[a.therapist] || TH_MAP["X"];
                      return (<tr key={a.id} style={{ background: i % 2 === 0 ? "white" : "#F0FAF7" }}>
                        <td style={{ padding: "3px 6px", borderBottom: "1px solid #D8EDE8" }}>{a.date}</td>
                        <td style={{ padding: "3px 6px", borderBottom: "1px solid #D8EDE8" }}>{a.time}</td>
                        <td style={{ padding: "3px 6px", borderBottom: "1px solid #D8EDE8", fontWeight: 600 }}>{a.patient}</td>
                        <td style={{ padding: "3px 6px", borderBottom: "1px solid #D8EDE8" }}><span style={{ display: "inline-block", width: 14, height: 14, borderRadius: "50%", background: th2.color, color: "white", textAlign: "center", lineHeight: "14px", fontSize: 8, fontWeight: 700, marginRight: 3 }}>{thLabel(th2)}</span>{th2.name}</td>
                        <td style={{ padding: "3px 6px", borderBottom: "1px solid #D8EDE8" }}>{getApptManualDur(a)}分</td>
                        <td style={{ padding: "3px 6px", borderBottom: "1px solid #D8EDE8" }}>{salaryUnlocked ? rev : "🔒"}</td>
                        <td style={{ padding: "3px 6px", borderBottom: "1px solid #D8EDE8", fontWeight: 700, color: "#2E7D6F" }}>{salaryUnlocked ? `+${Math.round(rev * 0.05)}` : "🔒"}</td>
                      </tr>);
                    })}</tbody>
                  </table>
                </div>}
  ...
  return (<div style={{ display: "flex", gap: 4, alignItems: "center", padding: "5px 12px", background: "#FFFDF5", borderRadius: 7, border: "1px solid #E0D5C1", marginBottom: 8, fontSize: showNames ? 12 : 10, flexWrap: "wrap" }}>
    ...
    {THERAPISTS.map(t => { const sel = filterTh === t.id; return (
      <button key={t.id} onClick={() => {
        ...
        <span style={{ width: showNames ? "auto" : "100%", textAlign: "center" }}>{thLabel(t)}</span>
  ...
  return (<div style={{ minHeight: "100vh", background: "#F8F2E6", fontFamily: "'Noto Sans TC', sans-serif" }}>
    ...
    <main style={{ maxWidth: 1100, margin: "0 auto", padding: "14px 10px" }}>
      ...
      {page === "front" && frontTab === "book" && (<>
        <div style={{ background: "#FFFDF5", border: "1.5px solid #E0D5C1", borderRadius: 10, padding: "14px 16px", marginBottom: 12, fontSize: 17, color: "#5A4A3A", lineHeight: 1.8 }}>
          ...
          <div style={{ paddingLeft: 4 }}>
            ...
            <div style={{ marginBottom: 4 }}><span style={{ color: "#C2563A", fontWeight: 700 }}>5.</span> 如果臨時無法前來，請記得於一天前完成線上取消。若多次未到且未取消，為了維護其他患者的權益，系統可能會暫停您的預約功能，謝謝您的配合與體諒。</div>
            <div><span style={{ color: "#C2563A", fontWeight: 700 }}>6.</span> 特殊需求可填備註（例：指定男／女老師），我們會視報到時現場狀況儘量安排。</div>
          </div>
        </div>
      ...
      {isAdmin && adminTab === "schedule" && (<><NavCtrl selDate={selDate} setSelDate={setSelDate} viewMode={adminView} setViewMode={setAdminView} showDayView={true} /><ThFilterBar filterTh={filterTh} setFilterTh={setFilterTh} /><div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 10, padding: "5px 12px", background: "#FFFDF5", borderRadius: 7, border: "1px solid #E0D5C1", fontSize: 13, alignItems: "center" }}>{THERAPISTS.map(t => (<span key={t.id} style={{ display: "flex", alignItems: "center", gap: 3 }}><span style={{ display: "inline-flex", width: 14, height: 14, borderRadius: "50%", background: t.color, color: "white", fontSize: 8, fontWeight: 700, alignItems: "center", justifyContent: "center" }}>{thLabel(t)}</span><span>{t.name}</span></span>))}<span style={{ borderLeft: "1px solid #D4C5A9", paddingLeft: 8, display: "flex", gap: 6 }}><span style={{ color: "#2E7D6F", fontWeight: 600 }}>■班內</span><span style={{ color: "#C2563A", fontWeight: 600 }}>┈班外</span><span style={{ color: "#FFD700", fontWeight: 600 }}>▮已報到</span></span></div>
        {copyMode && <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 16px", background: "#5B6ABF", color: "white", borderRadius: 8, marginBottom: 10, fontSize: 14, fontWeight: 600 }}>
