      ...
      {LU_SLOTS.map(time => { const isH = time.endsWith(":00"); const as = getStart(time); const aa = getAt(time); const occ = !!aa; const closed = isLuSlotClosed(ds, time, luSlotCfg);
        return (<tr key={time} style={{ cursor: "pointer" }}><td style={{ padding: "3px 8px", textAlign: "center", background: isH ? "#E8F5F0" : "#F2FAF7", borderRight: `2px solid ${LU_COLOR}30`, borderTop: isH ? `1px solid ${LU_COLOR}30` : "1px solid #E0EDE8", fontWeight: isH ? 700 : 400, color: isH ? LU_COLOR : "#6BA898", height: 38 }}>{time}</td>
          <td onClick={() => occ ? onApptClick(aa) : !closed && onCellClick(selDate, time)} style={{ padding: 0, height: 30, borderTop: isH ? `1px solid ${LU_COLOR}30` : "1px solid #E0EDE8", background: closed ? "#F5F0E5" : occ ? `${LU_COLOR}10` : "#F8FDFB" }}
            ...
            {as && <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "0 10px", height: "100%", borderLeft: as.checkedIn ? "3px solid #FFD700" : "3px solid transparent" }}><div style={{ width: 26, height: 26, borderRadius: "50%", background: LU_COLOR, flexShrink: 0, color: "white", fontWeight: 700, fontSize: 13, display: "flex", alignItems: "center", justifyContent: "center" }}>盧</div><span style={{ fontWeight: 700, color: "#3D2B1F" }}>{as.patient}</span><span style={{ color: "#8B7355", fontSize: 13 }}>{as.chartNum ? `#${as.chartNum}` : as.birthday}</span><span style={{ color: "#8B7355", fontSize: 13 }}>{as.duration}分</span>{as.checkedIn && <span style={{ fontSize: 12, fontWeight: 700, color: "#FFD700" }}>✓到</span>}</div>}
    ...
    <tbody>{LU_SLOTS.map((time) => { const isH = time.endsWith(":00"); return (<tr key={time}><td style={{ padding: "1px 2px", textAlign: "center", background: isH ? "#E8F5F0" : "#F2FAF7", position: "sticky", left: 0, zIndex: 5, borderRight: `2px solid ${LU_COLOR}40`, borderTop: isH ? `1px solid ${LU_COLOR}30` : "1px solid #E0EDE8", fontWeight: isH ? 700 : 400, color: isH ? LU_COLOR : "#6BA898", fontSize: isH ? 13 : 12, height: 28 }}>{time}</td>
      {wd.map((date, di) => {
        ...
        return (<td key={di} onClick={() => occ ? onApptClick(aa) : !closed && onCellClick(date, time)}
          ...
          onMouseEnter={e => { if (!occ && !closed) e.currentTarget.style.background = "#DDF0E8"; }} onMouseLeave={e => { if (!occ && !closed) e.currentTarget.style.background = "#F8FDFB"; }}>
          {as && (<div style={{ position: "absolute", top: 1, left: 1, right: 1, height: (as.duration / 15) * 28 - 2, background: `linear-gradient(135deg, ${LU_COLOR}EE, ${LU_COLOR}AA)`, borderRadius: 3, zIndex: 2, padding: "1px 3px", color: "white", fontSize: 11, fontWeight: 600, overflow: "hidden", display: "flex", flexDirection: "column", border: as.checkedIn ? "2px solid #FFD700" : "none" }}>
            <span>{as.patient.slice(0, 3)}</span>
            {as.duration >= 30 && <span style={{ fontSize: 10, opacity: 0.75 }}>{as.duration}m</span>}
          </div>)}
