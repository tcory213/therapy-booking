...
function weekDates(base) { const d = new Date(base), day = d.getDay(); const mon = new Date(d); mon.setDate(d.getDate() - day + (day === 0 ? -6 : 1)); return Array.from({ length: 7 }, (_, i) => { const x = new Date(mon); x.setDate(mon.getDate() + i); return x; }); }
...
function BookingForm({ date, time, appts, onBook, onClose, isAdmin, cs, mainSlotCfg, addExtra }) {
  const [patient, setPatient] = useState(""); const [bday, setBday] = useState(""); const [dur, setDur] = useState(15);
  ...
  const doBook = (data) => {
    if (!isAdmin) { setConfirmBook(data); return; }
    finalBook(data);
  };
  ...
  const submit = () => {
    ...
    const apptData = { id: Date.now(), date: ds, time, duration: curDur, therapist: selTh, patient: patient.trim(), birthday: bday.trim(), onDuty, selfRef, treatType };
  ...
  return (<div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
    ...
    <ConfirmModal open={!!confirmBook} message="請再次確認預約時間、治療時長、治療師姓名。確認後若需修改或取消須來電。謝謝" onOk={() => { finalBook(confirmBook); setConfirmBook(null); }} onCancel={() => setConfirmBook(null)} />
  </div>);
}
...
function LuBookingForm({ date, time, appts, onBook, onClose, isAdmin, luSlotCfg }) {
  ...
  const finalBook = (data) => { onBook(data); onClose(); };
  const doBook = () => {
    if (!patient.trim()) { setErr("請輸入患者姓名"); return; } if (!bday.trim() || bday.length !== 6) { setErr("請輸入民國年月日六碼"); return; }
    if (!luValidRange(time, dur)) { setErr("超出盧獨立時段"); return; }
    const data = { id: Date.now(), date: ds, time, duration: dur, patient: patient.trim(), birthday: bday.trim(), selfRef };
    if (!isAdmin) { setConfirmBook(data); return; }
    finalBook(data);
  };
  const submit = () => {
    if (!patient.trim()) { setErr("請輸入患者姓名"); return; } if (!bday.trim() || bday.length !== 6) { setErr("請輸入民國年月日六碼"); return; }
    if (isAdmin && hasBuf && !occupied) { setConfirmBuf({ patient: patient.trim(), birthday: bday.trim() }); return; }
    doBook();
  };
  ...
  return (<div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
    ...
    <ConfirmModal open={!!confirmBuf} message="此預約違反 15 分鐘間隔規定，確定仍要預約嗎？" onOk={() => { setConfirmBuf(null); finalBook({ id: Date.now(), date: ds, time, duration: dur, patient: confirmBuf.patient, birthday: confirmBuf.birthday, selfRef }); }} onCancel={() => setConfirmBuf(null)} />
    <ConfirmModal open={!!confirmBook} message="請再次確認預約時間、治療時長。確認後若需修改或取消須來電。謝謝" onOk={() => { finalBook(confirmBook); setConfirmBook(null); }} onCancel={() => setConfirmBook(null)} />
  </div>);
}
...
function AdminDetail({ appt, appts, onClose, onDelete, onUpdate, onAlert, onCopyDates, onAddExtra }) {
  ...
  const doSave = () => { onUpdate(appt.id, { therapist: th, onDuty, duration: effectiveDur, selfRef, treatType }); setEditing(false); };
  ...
  return (<div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
    ...
    {!editing && !showCopy && <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
      ...
      <button onClick={() => setShowCopy(true)} style={{ flex: 1, padding: 9, borderRadius: 7, border: "1.5px solid #5B6ABF", background: "#EEEEFF", color: "#5B6ABF", cursor: "pointer", fontWeight: 600, fontSize: 11, fontFamily: "'Noto Sans TC', sans-serif" }}>📋 複製</button>
      <button onClick={() => { onAddExtra(appt); onClose(); }} style={{ flex: 1, padding: 9, borderRadius: 7, border: "1.5px solid #B8860B", background: "#FFF8E6", color: "#B8860B", cursor: "pointer", fontWeight: 600, fontSize: 11, fontFamily: "'Noto Sans TC', sans-serif" }}>外加</button>
...
function FrontWeekGrid({ appts, selDate, onCellClick, mainSlotCfg, filterTh, cs }) {
  ...
  const renderMap = useMemo(() => { const map = {}; for (let di = 0; di < 7; di++) { const col = dayData[di]; let i = 0; while (i < col.length) { if (col[i].blocked) { let j = i; while (j < col.length && col[j].blocked) j++; map[`${i}-${di}`] = { render: true, span: j - i, type: "blocked" }; for (let k = i + 1; k < j; k++) map[`${k}-${di}`] = { render: false }; i = j; } else { map[`${i}-${di}`] = { render: true, span: 1, type: "free", time: col[i].time, dimmed: col[i].dimmed }; i++; } } } return map; }, [dayData]);
  return (<div style={{ overflowX: "auto", borderRadius: 9, border: "1px solid #E0D5C1" }}><table style={{ borderCollapse: "collapse", width: "100%", minWidth: 520, fontSize: 13, fontFamily: "'Noto Sans TC', sans-serif" }}>
    <thead><tr><th style={{ padding: "8px 4px", background: "#3D2B1F", color: "#F5EDDC", position: "sticky", left: 0, zIndex: 10, minWidth: 48, width: 48, borderRight: "2px solid #5A4A3A", fontSize: 13 }}>時間</th>
      {wd.map((date, di) => { const isT = dsArr[di] === today; return (<th key={di} style={{ padding: "6px 3px", background: isT ? "#5A3A2A" : "#3D2B1F", color: "#F5EDDC", borderLeft: "1px solid #5A4A3A", fontSize: 12, minWidth: 58, width: "13%" }}><div>{`${date.getMonth() + 1}/${date.getDate()}`}</div><div style={{ fontSize: 12, color: isT ? "#F0A080" : "#C4B49A", marginTop: 1 }}>{WDAY[di]}{isT && " 今"}</div></th>); })}</tr></thead>
...
function LuFrontWeekGrid({ appts, selDate, onCellClick, luSlotCfg }) {
  ...
  const renderMap = useMemo(() => { const map = {}; for (let di = 0; di < 7; di++) { const col = dayData[di]; let i = 0; while (i < col.length) { if (col[i].blocked) { let j = i; while (j < col.length && col[j].blocked) j++; map[`${i}-${di}`] = { render: true, span: j - i, type: "blocked" }; for (let k = i + 1; k < j; k++) map[`${k}-${di}`] = { render: false }; i = j; } else { map[`${i}-${di}`] = { render: true, span: 1, type: "free", time: col[i].time }; i++; } } } return map; }, [dayData]);
  return (<div style={{ overflowX: "auto", borderRadius: 9, border: `1px solid ${LU_COLOR}40` }}><table style={{ borderCollapse: "collapse", width: "100%", minWidth: 520, fontSize: 13, fontFamily: "'Noto Sans TC', sans-serif" }}>
    <thead><tr><th style={{ padding: "8px 4px", background: LU_COLOR, color: "white", position: "sticky", left: 0, zIndex: 10, minWidth: 48, width: 48, borderRight: `2px solid ${LU_COLOR}`, fontSize: 13 }}>時間</th>
      {wd.map((date, di) => { const isT = dsArr[di] === today; return (<th key={di} style={{ padding: "6px 3px", background: isT ? "#1D8068" : LU_COLOR, color: "white", borderLeft: `1px solid ${LU_COLOR}88`, fontSize: 12, minWidth: 58, width: "13%" }}><div>{`${date.getMonth() + 1}/${date.getDate()}`}</div><div style={{ fontSize: 12, color: isT ? "#A0E8D0" : "#88CCB8", marginTop: 1 }}>{WDAY[di]}{isT && " 今"}</div></th>); })}</tr></thead>
...
/* ═══════════════════════════════════════════ Front Phone Lookup ═══════════════════════════════════════════ */
function PhoneLookup({ appts, luAppts }) {
  const [bday, setBday] = useState("");
  const results = useMemo(() => { if (bday.length !== 6) return []; const today = fd(new Date()); const mainR = appts.filter(a => a.onDuty && a.birthday === bday && a.date >= today).map(a => ({ ...a, sys: "main" })); const luR = luAppts.filter(a => a.birthday === bday && a.date >= today).map(a => ({ ...a, sys: "lu" })); return [...mainR, ...luR].sort(sortByDateTime); }, [bday, appts, luAppts]);
  return (<div style={{ maxWidth: 500, margin: "0 auto" }}>
    <div style={{ textAlign: "center", marginBottom: 20 }}><div style={{ fontSize: 42, marginBottom: 8 }}>🔍</div><h2 style={{ margin: 0, fontFamily: "'Noto Serif TC', serif", color: "#3D2B1F", fontSize: 21 }}>預約查詢</h2><p style={{ color: "#8B7355", fontSize: 15, margin: "6px 0 0 0" }}>輸入生日六碼查詢今日起的預約</p></div>
    <input value={bday} onChange={e => setBday(e.target.value.replace(/\D/g, "").slice(0, 6))} placeholder="輸入民國年月日六碼" style={{ width: "100%", padding: "14px 18px", borderRadius: 10, border: "1.5px solid #D4C5A9", fontSize: 19, background: "#FFFDF5", fontFamily: "'Noto Sans TC', sans-serif", boxSizing: "border-box", outline: "none", textAlign: "center", letterSpacing: 2 }} />
    {bday.length > 0 && bday.length < 6 && <p style={{ textAlign: "center", fontSize: 14, color: "#B5A898", margin: "8px 0 0 0" }}>請輸入完整六碼</p>}
    {bday.length === 6 && results.length === 0 && <div style={{ textAlign: "center", padding: "30px 0", color: "#8B7355" }}><div style={{ fontSize: 38, marginBottom: 8 }}>📭</div><p style={{ margin: 0, fontSize: 16 }}>查無今日以後的預約紀錄</p></div>}
    {results.length > 0 && <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 8 }}><p style={{ fontSize: 15, color: "#8B7355", margin: 0 }}>找到 {results.length} 筆</p>
      {results.map(a => { const isLu = a.sys === "lu"; const thObj = isLu ? null : (TH_MAP[a.therapist] || TH_MAP["X"]); const color = isLu ? LU_COLOR : thObj.color; return (<div key={a.id} style={{ background: "#FFFDF5", border: `1.5px solid ${color}40`, borderRadius: 10, padding: 16 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, fontSize: 15 }}><div><span style={{ color: "#8B7355" }}>日期：</span><strong>{a.date}</strong></div><div><span style={{ color: "#8B7355" }}>時間：</span><strong>{a.time}</strong></div><div><span style={{ color: "#8B7355" }}>時長：</span>{a.duration} 分鐘</div><div><span style={{ color: "#8B7355" }}>患者：</span>{maskN(a.patient)}</div></div></div>); })}</div>}
  </div>);
}
...
function AdminWeekGrid({ appts, selDate, onCellClick, onApptClick, filterTh, cs }) {
  ...
  return (<div style={{ overflowX: "auto", borderRadius: 9, border: "1px solid #E0D5C1" }}><table style={{ borderCollapse: "collapse", width: "100%", minWidth: 520, fontSize: 12, fontFamily: "'Noto Sans TC', sans-serif" }}>
    <thead><tr><th style={{ padding: "7px 4px", background: "#3D2B1F", color: "#F5EDDC", position: "sticky", left: 0, zIndex: 10, minWidth: 48, width: 48, borderRight: "2px solid #5A4A3A", fontSize: 12 }}>時間</th>{wd.map((date, di) => { const isT = dsArr[di] === todayStr; return (<th key={di} style={{ padding: "5px 3px", background: isT ? "#5A3A2A" : "#3D2B1F", color: "#F5EDDC", borderLeft: "1px solid #5A4A3A", fontSize: 11, minWidth: 58, width: "13%" }}><div>{`${date.getMonth() + 1}/${date.getDate()}`}</div><div style={{ fontSize: 11, color: isT ? "#F0A080" : "#C4B49A", marginTop: 1 }}>{WDAY[di]}{isT && " 今"}</div></th>); })}</tr></thead>
...
function AdminDayView({ appts, selDate, onApptClick, onCellClick, mainSlotCfg, setMainSlotCfg }) {
  ...
  return (<div>
    <div style={{ display: "flex", gap: 12, marginBottom: 10, padding: "8px 14px", background: "#FFFDF5", borderRadius: 7, border: "1px solid #E0D5C1", fontSize: 11, color: "#5A4A3A", flexWrap: "wrap", alignItems: "center" }}>
      ...
      <button onClick={() => setShowPrint(true)} style={{ marginLeft: "auto", padding: "5px 14px", borderRadius: 6, border: "1.5px solid #3D2B1F", background: "#FFFDF5", color: "#3D2B1F", cursor: "pointer", fontWeight: 600, fontSize: 11, fontFamily: "'Noto Sans TC', sans-serif" }}>🖨️ 列印</button>
    </div>
      ...
      {SLOTS.map(time => { const isH = time.endsWith(":00"); const isBr = time === "14:00"; const starts = getStarts(time); const all = getAllAt(time); const occ = all.length > 0;
        ...
        return (<tr key={time} style={{ cursor: "pointer", ...(isBr ? { borderTop: "3px solid #C2563A" } : {}) }}><td onClick={() => !occ && !closed && onCellClick(selDate, time)} style={{ padding: "3px 8px", textAlign: "center", background: isH ? "#F0E8D8" : "#F8F2E6", borderRight: "2px solid #D4C5A9", borderTop: isH ? "1px solid #D4C5A9" : "1px solid #EDE5D5", fontWeight: isH ? 700 : 400, color: isH ? "#3D2B1F" : "#8B7355", height: 30 }}>{time}</td>
          <td style={{ padding: 0, minHeight: 30, borderTop: isH ? "1px solid #D4C5A9" : "1px solid #EDE5D5", background: closed ? "#F5F0E5" : occ ? "#FAFAF5" : "#FFFDF5" }} onMouseEnter={e => { if (!occ && !closed) e.currentTarget.style.background = "#F0E0C8"; }} onMouseLeave={e => { if (!occ && !closed) e.currentTarget.style.background = closed ? "#F5F0E5" : "#FFFDF5"; }}>
            {starts.map(as => { const th = TH_MAP[as.therapist] || TH_MAP["X"]; return (<div key={as.id} onClick={() => onApptClick(as)} style={{ display: "flex", alignItems: "center", gap: 6, padding: "3px 10px", cursor: "pointer", borderLeft: as.checkedIn ? "3px solid #FFD700" : "3px solid transparent" }}><div style={{ width: 20, height: 20, borderRadius: "50%", background: th.color, flexShrink: 0, color: "white", fontWeight: 700, fontSize: 10, display: "flex", alignItems: "center", justifyContent: "center" }}>{th.id === "X" ? "?" : th.id}</div><span style={{ fontWeight: 700, color: "#3D2B1F" }}>{as.patient}</span><span style={{ color: "#8B7355", fontSize: 10 }}>{as.birthday}</span><span style={{ color: "#8B7355", fontSize: 10 }}>{as.duration}分</span><span style={{ fontSize: 9, fontWeight: 600, padding: "1px 5px", borderRadius: 3, background: "#F5F0E5", color: "#5A4A3A" }}>{TREAT_MAP[as.treatType] || "徒手"}</span><span style={{ fontSize: 9, fontWeight: 600, padding: "1px 5px", borderRadius: 3, background: as.onDuty ? "#E6F5EE" : "#FFF0EB", color: as.onDuty ? "#2E7D6F" : "#C2563A" }}>{as.onDuty ? "班內" : "班外"}</span><span style={{ fontSize: 9, fontWeight: 600, padding: "1px 5px", borderRadius: 3, background: as.selfRef ? "#EEEEFF" : "#FFF8E6", color: as.selfRef ? "#5B6ABF" : "#B8860B" }}>{as.selfRef ? "自轉" : "非自轉"}</span>{as.checkedIn && <span style={{ fontSize: 9, fontWeight: 700, color: "#FFD700" }}>✓到</span>}</div>); })}
            ...
            {!occ && closed && <div style={{ padding: "0 10px", height: 30, display: "flex", alignItems: "center", fontSize: 10, color: "#B5A898" }}>已關閉</div>}
            {!occ && !closed && <div onClick={() => onCellClick(selDate, time)} style={{ padding: "0 10px", height: 30, display: "flex", alignItems: "center", fontSize: 10, color: "#C4B49A" }}>可預約</div>}
          </td>
...
function LuAdminDayView({ appts, selDate, onCellClick, onApptClick, luSlotCfg, setLuSlotCfg }) {
  ...
  return (<div>
    <div style={{ display: "flex", gap: 12, marginBottom: 10, padding: "8px 14px", background: "#F8FDFB", borderRadius: 7, border: `1px solid ${LU_COLOR}30`, fontSize: 11, color: "#5A4A3A", flexWrap: "wrap", alignItems: "center" }}>
      ...
      {LU_SLOTS.map(time => { const isH = time.endsWith(":00"); const as = getStart(time); const aa = getAt(time); const occ = !!aa; const closed = luSlotCfg[`${ds}-${time}`] === false;
        return (<tr key={time} style={{ cursor: "pointer" }}><td style={{ padding: "3px 8px", textAlign: "center", background: isH ? "#E8F5F0" : "#F2FAF7", borderRight: `2px solid ${LU_COLOR}30`, borderTop: isH ? `1px solid ${LU_COLOR}30` : "1px solid #E0EDE8", fontWeight: isH ? 700 : 400, color: isH ? LU_COLOR : "#6BA898", height: 30 }}>{time}</td>
          <td onClick={() => occ ? onApptClick(aa) : !closed && onCellClick(selDate, time)} style={{ padding: 0, height: 30, borderTop: isH ? `1px solid ${LU_COLOR}30` : "1px solid #E0EDE8", background: closed ? "#F5F0E5" : occ ? `${LU_COLOR}10` : "#F8FDFB" }}
            ...
            {!occ && closed && <div style={{ padding: "0 10px", height: "100%", display: "flex", alignItems: "center", fontSize: 10, color: "#B5A898" }}>已關閉</div>}
            {!occ && !closed && <div style={{ padding: "0 10px", height: "100%", display: "flex", alignItems: "center", fontSize: 10, color: "#88CCB8" }}>可預約</div>}
          </td>
...
function LuAdminWeekGrid({ appts, selDate, onCellClick, onApptClick, luSlotCfg }) {
  ...
  return (<div style={{ overflowX: "auto", borderRadius: 9, border: `1px solid ${LU_COLOR}40` }}><table style={{ borderCollapse: "collapse", width: "100%", minWidth: 480, fontSize: 10, fontFamily: "'Noto Sans TC', sans-serif" }}>
    <thead><tr><th style={{ padding: "7px 3px", background: LU_COLOR, color: "white", position: "sticky", left: 0, zIndex: 10, minWidth: 42, width: 42, borderRight: `2px solid ${LU_COLOR}`, fontSize: 10 }}>時間</th>
      {wd.map((date, di) => { const isT = dsArr[di] === todayStr; return (<th key={di} style={{ padding: "5px 2px", background: isT ? "#1D8068" : LU_COLOR, color: "white", borderLeft: `1px solid ${LU_COLOR}88`, fontSize: 9, minWidth: 52, width: "13%" }}><div>{`${date.getMonth() + 1}/${date.getDate()}`}</div><div style={{ fontSize: 9, color: isT ? "#A0E8D0" : "#88CCB8", marginTop: 1 }}>{WDAY[di]}{isT && " 今"}</div></th>); })}</tr></thead>
...
function ShiftEditor({ customShifts, setCustomShifts }) {
  ...
  return (<div>
    <div style={{ display: "flex", gap: 8, marginBottom: 14, flexWrap: "wrap", alignItems: "center" }}>
      ...
      <button onClick={() => { const u = {}; dates.forEach(({ ds }) => { u[`${selTh}-${ds}`] = []; }); setCustomShifts(prev => ({ ...prev, ...u })); }} style={{ padding: "6px 12px", borderRadius: 6, border: "1.5px solid #C2563A", background: "#FFF0EB", color: "#C2563A", cursor: "pointer", fontWeight: 600, fontSize: 11, fontFamily: "'Noto Sans TC', sans-serif" }}>全月清除</button>
    </div>
    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10, padding: "8px 14px", background: `${th.color}12`, borderRadius: 8, border: `1.5px solid ${th.color}30` }}><div style={{ width: 28, height: 28, borderRadius: "50%", background: th.color, color: "white", fontWeight: 700, fontSize: 14, display: "flex", alignItems: "center", justifyContent: "center" }}>{th.id}</div><span style={{ fontWeight: 700, color: "#3D2B1F", fontSize: 14 }}>{th.name}</span><span style={{ fontSize: 10, color: "#8B7355", marginLeft: "auto" }}>無班 → 班內 → 班外 → 無班</span></div>
    <div style={{ display: "flex", gap: 14, marginBottom: 10, fontSize: 11, color: "#5A4A3A" }}><span style={{ display: "flex", alignItems: "center", gap: 4 }}><span style={{ display: "inline-block", width: 16, height: 16, borderRadius: 3, background: `${th.color}DD` }} /> 班內</span><span style={{ display: "flex", alignItems: "center", gap: 4 }}><span style={{ display: "inline-block", width: 16, height: 16, borderRadius: 3, background: `${th.color}40` }} /> 班外</span><span style={{ display: "flex", alignItems: "center", gap: 4 }}><span style={{ display: "inline-block", width: 16, height: 16, borderRadius: 3, background: "#EDE5D5" }} /> 無班</span></div>
    <div style={{ background: "#FFFDF5", borderRadius: 10, border: "1px solid #E0D5C1", overflow: "hidden" }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", background: "#3D2B1F" }}>{["日", "一", "二", "三", "四", "五", "六"].map(d => (<div key={d} style={{ padding: "8px 0", textAlign: "center", color: "#F5EDDC", fontSize: 12, fontWeight: 600 }}>{d}</div>))}</div>
    ...
    <header style={{ background: "linear-gradient(135deg, #3D2B1F, #5A3A2A)", padding: "10px 18px", boxShadow: "0 4px 20px rgba(0,0,0,0.15)" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
        ...
        <div style={{ display: "flex", gap: 3, alignItems: "center", flexWrap: "wrap" }}>
          {isAdmin ? (<>
            {[{ k: "schedule", l: "排程表" }, { k: "lu", l: "盧獨立時段" }, { k: "salary", l: "薪資" }, { k: "shifts", l: "班表" }].map(t => (<button key={t.k} onClick={() => setAdminTab(t.k)} style={{ padding: "5px 10px", borderRadius: 5, border: "none", cursor: "pointer", background: adminTab === t.k ? (t.k === "lu" ? LU_COLOR : "#C2563A") : "rgba(255,255,255,0.1)", color: adminTab === t.k ? "white" : "#C4B49A", fontWeight: 600, fontSize: 11, fontFamily: "'Noto Sans TC', sans-serif" }}>{t.l}</button>))}
            <div style={{ width: 1, height: 18, background: "#5A4A3A", margin: "0 4px" }} /><button onClick={() => setPage("front")} style={{ padding: "5px 10px", borderRadius: 5, border: "1px solid #C4B49A55", background: "transparent", color: "#C4B49A", cursor: "pointer", fontSize: 10 }}>返回前台</button>
          </>) : (<>
            {[{ k: "book", l: "📅 預約" }, { k: "lookup", l: "🔍 查詢" }].map(t => (<button key={t.k} onClick={() => setFrontTab(t.k)} style={{ padding: "6px 12px", borderRadius: 5, border: "none", cursor: "pointer", background: frontTab === t.k ? "#C2563A" : "rgba(255,255,255,0.1)", color: frontTab === t.k ? "white" : "#C4B49A", fontWeight: 600, fontSize: 14, fontFamily: "'Noto Sans TC', sans-serif" }}>{t.l}</button>))}
    ...
    <main style={{ maxWidth: 1100, margin: "0 auto", padding: "14px 10px" }}>
      ...
      {page === "front" && frontTab === "book" && (<>
        <div style={{ background: "#FFFDF5", border: "1.5px solid #E0D5C1", borderRadius: 10, padding: "14px 16px", marginBottom: 12, fontSize: 13, color: "#5A4A3A", lineHeight: 1.8 }}>
          <div style={{ fontWeight: 700, color: "#3D2B1F", fontSize: 15, marginBottom: 8, fontFamily: "'Noto Serif TC', serif" }}>📋 預約說明</div>
      ...
      {page === "front" && frontTab === "lookup" && <div style={{ paddingTop: 20 }}><PhoneLookup appts={appts} luAppts={luAppts} /></div>}
